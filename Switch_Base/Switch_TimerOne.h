 ...
 */
#ifndef SWITCH_TIMERONE
#define SWITCH_TIMERONE
...
namespace Switch
{
  ...
  public:
    ...
    // methods
    void Begin (const uint32_t& i_periodUs=1000000);
    void SetPeriod (const uint32_t& i_periodUs);
    ...
    void PWM (uint8_t i_pin, uint16_t i_duty, const uint32_t& i_periodUs=0);
    void SetPWMDuty (uint8_t i_pin, uint16_t i_duty);
    void DisablePWM (uint8_t i_pin);

    void AttachInterrupt (void (*i_pISR) ());
  ...
  private:
    ...
    // properties
    uint8_t   m_clockSelectBits;

  };
}

extern Switch::TimerOne Timer1;

#endif // ARDUINO

#endif // SWITCH_TIMERONE
